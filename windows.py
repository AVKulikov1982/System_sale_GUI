...
class StartWindow:
    def __init__(self, results=None, width=250, height=150,
                 title='Start_system_sales', c_rows=4, c_columns=2, params=None):
        self.root = Tk()
        self.title = title
        ...
        self.chek_save = IntVar()
        self.chek = Checkbutton(self.root,
                                text='Запомнить',
                                variable=self.chek_save,
                                width=9)
        self.chek.select()
        self.check_file = os.path.exists('login_pass.txt')

        self.entry_login = Entry(self.root)
        self.entry_password = Entry(self.root, show='*')
        if self.check_file:
            with open('login_pass.txt', 'r', encoding='utf8') as f_o_read:
                string = f_o_read.read()
                f_o_read.close()
                login = string.split('\n')[0]
    ...
    def get_pdf_payments(self):
        self.results = get_payments()

    def get_registration(self):
        login = self.entry_login.get()
        password = self.entry_password.get()
        if self.chek_save.get():
            with open('login_pass.txt', 'w', encoding='utf8') as f_o_write:
                string = f'{login}\n{password}'
                f_o_write.write(string)
                f_o_write.close()
        else:
            if self.check_file:
                os.remove('login_pass.txt')
        try:
...
class ShowResult(StartWindow):
    ...
    def export_to_excel(self):
        ...
        df = DataFrame(self.list_for_export)
...
class UpdateWindow(ShowResult):
    ...
    def check_update(self):
        ...
        # TODO продумать на дубли pdf
        # TODO разобраться с иконками
        # TODO добавить "редактировать prices"
